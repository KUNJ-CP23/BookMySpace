...
export default function Payments() {
  ...
  const fetchPayments = async () => {
    ...
    try {
      ...
      console.log("Payments Error:", err.response?.data || err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const statusBadge = (status) => {
    ...
    const cls = map[status] ?? "bg-gray-100 text-gray-500 dark:bg-zinc-800 dark:text-zinc-400";
    return (
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-5">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search by user, facility or method…"
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-800/60">
                  <th className="text-left px-5 py-3.5 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-zinc-400">User</th>
                  <th className="text-left px-5 py-3.5 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-zinc-400">Facility</th>
                  <th className="text-left px-5 py-3.5 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-zinc-400">Amount</th>
                  <th className="text-left px-5 py-3.5 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-zinc-400">Method</th>
                  ...
                  <th className="text-left px-5 py-3.5 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-zinc-400">Paid On</th>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-zinc-800">
                {filteredPayments.length === 0 ? (
                  ...
                  <tr
                    key={p.paymentId}
                    className="hover:bg-blue-50/40 dark:hover:bg-zinc-800/50 transition-colors duration-100"
                  >
                    {/* User */}
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-medium text-gray-900 dark:text-zinc-100">{p.user || "—"}</span>
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          onClick={() => navigate(`/payments/form/${p.paymentId}`)}
